...
- **時間帯別ポーリング回数**: `schedule` に 24 要素の配列（回/時）で設定。正時 :00 を起点に等間隔でポーリング。最低 1 回/時が保証される（1 未満の値は 1 として扱う）
- **スリープ復帰・ロック解除時の即時ポーリング**: PC のスリープ復帰またはロック解除時に即座にポーリングを実行。前回から 1 時間以上経過した場合も即時ポーリング。連続トリガーを防ぐため前回ポーリングから 60 秒以内の場合はスキップ
- **リッチ Toast 通知**: アプリアイコン（app.ico）を通知に表示。OS 通知音を無効化して独自チャイムとの重複を防止。「Calendar」ボタンからイベントページを直接開ける
- **通知音**: exe 同フォルダの `audio1.opus`（チャイム音）を ffplay で再生後、`audio2.opus` があれば続けて再生。音声ファイルがない場合は Toast 通知のみ表示
...
- **設定ホットリロード**: 5分間隔で TOML 設定を再読み込み。プロセス再起動なしに設定変更を反映
- **システムトレイアイコン**: タスクトレイにアイコンを常駐。ホバーで「本日の以降予定：N 件」のように当日の残り予定件数を表示（左クリックやメニュー表示中はツールチップを抑制）。左クリックで当日の予定一覧をポップアップ表示（クリックで Google Calendar のイベントページを開く）。右クリックのコンテキストメニューから音声通知設定（「音声通知」ON/OFF・「次回のみ音声通知無効」・「マイク/カメラ使用中は無効」）・「設定ファイル」（OSデフォルトエディタで開く）・「ログファイル」（当日ログをエディタで開く）・「再起動」・「終了」が可能。エクスプローラ再起動時もアイコンを自動復帰
...
```toml
# 0時〜23時のポーリング回数（回/時、最低 1）。正時起点の等間隔。1 未満の値は 1 として扱う
