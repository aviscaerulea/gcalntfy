...
トレイアイコンは、以降予定がある場合に右下へバッジを表示します。カーソルを乗せたままにすると（デフォルト 0.1 秒、`hover_delay_ms` で調整可能）、左クリックと同じ予定一覧を表示します。

予定一覧はフォーカスを奪わないため、直前まで使っていたウィンドウでの入力を妨げません。

予定一覧は、カーソルがアイコンと一覧の外に出ると自動で閉じます。左クリックは開閉のトグルとして働き、表示中なら閉じ、非表示なら即座に開きます。ただしホバーで自動表示した直後（デフォルト 0.3 秒、`hover_click_guard_ms` で調整可能）の左クリックは無視します。一覧を出すつもりのクリック直前に自動表示が割り込み、クリックが「閉じる」に化けるのを防ぐためです。

一覧を閉じた後は、カーソルがアイコンから離れるまでホバーでの再表示を行いません。閉じた直後のわずかな動きで開き直すのを防ぐためです。

ホバー表示の ON/OFF は、右クリックメニューの「マウスホバーで一覧を自動表示」で切り替えられます。OFF にしても左クリックでは従来どおり開けます。（デフォルト ON、設定は再起動後も維持）

予定一覧では、フッタークリックで Google Calendar の当日ページを開き、予定項目の右クリックで通知抑制をトグルできます。予定一覧はマウス専用で、キーボードでは操作できません。
...
予定のキャンセルを検知したときは、次の Toast 通知を表示します。

![予定のキャンセルを検知したときの Toast 通知](docs/images/cancel-toast.png)

「今すぐ更新」の成功時は、次の Toast 通知を表示します。
