...
トレイアイコンは、以降予定がある場合に右下へバッジを表示します。カーソルを乗せたままにすると（デフォルト 0.2 秒、`hover_delay_ms` で調整可能）、予定一覧を表示します。
...
ホバー表示が ON の間は、残り予定件数のツールチップを表示しません。同じホバー操作で予定一覧と重なるためです。OFF にすると、ホバーで残り予定件数を確認でき、予定一覧は左クリックで表示します。

ホバー表示が ON の間、左クリックは何もしません。（未認証の状態で認証を始める場合を除く）

一覧を閉じた後は、カーソルを一度アイコンから離すまで再表示しません。閉じた直後の小さなカーソル移動で開き直すのを防ぐためです。

予定一覧では、フッタークリックで Google Calendar の当日ページを開き、予定項目の右クリックで通知抑制をトグルできます。
...
予定一覧で予定項目を右クリックすると通知抑制をトグルできます。
...
# urgent_minutes = 60
# ホバーで予定一覧を表示するまでの遅延（ミリ秒、0〜5000、デフォルト 200、0 で即時）
# hover_delay_ms = 200
