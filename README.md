...
- **システムトレイアイコン**: タスクトレイにアイコンを常駐。ホバーで「この後の予定：N 件」のように当日の残り予定件数を表示（左クリックやメニュー表示中はツールチップを抑制）。左クリックで当日の予定一覧をポップアップ表示（クリックで Google Calendar のイベントページを開く）。右クリックのコンテキストメニューから音声通知設定（「音声通知」ON/OFF・「次回のみ音声通知無効」・「マイク/カメラ使用中は無効」）・「設定ファイル」（OSデフォルトエディタで開く）・「ログファイル」（当日ログをエディタで開く）・「再起動」・「終了」が可能。エクスプローラ再起動時もアイコンを自動復帰
