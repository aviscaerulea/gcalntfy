...
予定一覧の各項目には、開始までの残り時間を「（n時間n分後）」形式で表示します。開始が迫った予定は赤文字、次の予定は太字で強調します。フッタークリックで Google カレンダーの当日ページを開けます。予定項目の右クリックで、その予定の通知を止められます。
...
予定一覧で予定を右クリックすると、その予定の通知が止まります。もう一度右クリックすると元に戻ります。

| 項目 | 動作 |
|---|---|
| 止まる範囲 | 右クリックした予定だけ（毎週などの繰り返す予定は、その日の分だけ止まる） |
| 止まる通知 | 予定の開始前と、Google カレンダー側で設定した時刻に出る通知 |
| 止まらない通知 | 予定の変更・キャンセル・追加を見つけたときの通知（大事な知らせのため必ず届く） |
| 設定の保存 | アプリを終了しても残る（過ぎた日の分は次の起動時に消える） |
| 見た目 | 通知を止めた予定に取消線が付く |
