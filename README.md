...
- 次のイベントの 4 分前に Windows Toast 通知を表示し、チャイム音（intro.opus）を再生。同フォルダに gcalntfy.opus があれば続けて再生
- 日付が変わると通知済みセットをリセットして当日分を再取得

### 特徴

- **時間帯別ポーリング回数**: `schedule` に 24 要素の配列（回/時）で設定。正時 :00 を起点に等間隔でポーリング。`0` の時間帯はポーリングしない
- **リッチ Toast 通知**: アプリアイコン（app.ico）を通知に表示。OS 通知音を無効化して独自チャイムとの重複を防止。「Calendar」ボタンからイベントページを直接開ける
- **通知音**: exe に埋め込んだチャイム音（intro.opus）を ffplay で再生後、同フォルダの `gcalntfy.opus` があれば続けて再生
...
- **システムトレイアイコン**: タスクトレイにアイコンを常駐。ホバーで次の予定を表示。左クリックで当日の予定一覧をポップアップ表示（クリックで Google Calendar のイベントページを開く）。右クリックのコンテキストメニューから音声通知設定（「音声通知」ON/OFF・「次回のみ音声通知無効」・「マイク/カメラ使用中は無効」）・「設定ファイル」（OSデフォルトエディタで開く）・「再起動」・「終了」が可能。エクスプローラ再起動時もアイコンを自動復帰
