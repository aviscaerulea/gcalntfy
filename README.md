...
- OAuth 2.0 で認証した Google Calendar（時刻指定の繰り返さないタスクを含む）をポーリングし、予定開始前や変更を Toast 通知と音声で知らせる
- 時間帯別ポーリング回数：`schedule` に 24 要素の配列（回/時）で各時間帯のポーリング頻度を設定可能
- 即時ポーリング：PC スリープ復帰・セッションロック解除・ネットワーク復帰時に即座にポーリングを実行
- 今すぐ更新：トレイメニューから任意のタイミングで即時ポーリングを実行し、完了・失敗を Toast 通知で知らせる（未認証・認証中は選択不可）
...
- 過去予定のグレー表示：当日の過去予定も予定一覧にグレー表示で残り、クリックで予定ページを開ける
- 過去予定の表示切替：トレイメニューから過去予定の表示 ON/OFF を切り替え可能
- 音声設定の永続化：音声通知と「マイク/カメラ使用中は無効」の設定は再起動後も維持
- 起動時キャッシュ復元：直前のポーリング結果を `events.json` にキャッシュし、起動直後にネットワーク不通でもポーリング成功時の予定データを表示
- 変更検知通知：予定の日時変更・キャンセル・新規追加を検知し Toast 通知で報告
...
トレイアイコンは、以降予定がある場合に右下へバッジを表示し、ホバーで残り予定件数を確認できます。
...
当日の開始時刻を過ぎた予定は、終日予定を除いて予定一覧にグレー表示で残り、クリックで予定ページを開けます。バッジ・ホバー・フッターの件数は今後の予定のみを数えます。
...
`gcalntfy.toml`（または `gcalntfy.local.toml`）を exe と同フォルダに配置してください。
