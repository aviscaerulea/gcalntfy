...
- 予定の通知：Google カレンダーをポーリングし、予定の開始前や予定の変更を Windows 通知で知らせる（開始前の通知は音声も鳴らす）
...
| 予定の開始直前（デフォルト 60 秒前） | ✓ | ✓ | トレイメニューの「直前通知を行う」が ON の場合のみ（デフォルト OFF、設定で秒数を 0〜60 に調整でき音声も止められる） |
